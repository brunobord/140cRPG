#!/usr/bin/env python
#-*- coding: utf-8 -*-
"""140c RPG building document script.
...
BUILD_PATH = os.path.abspath(os.path.join('.', 'www'))
    ...
    def __init__(self, fragment_path):
        ...
        for filename in os.listdir(fragment_path):
            ...
            if filename.endswith('.md'):
                self.fragments[filename] = codecs.open(os.path.join(
                    fragment_path, filename), encoding='utf').read()
...
class HTMLWriter(object):
    ...
    def write(self, base, data):
        ...
        if base != 'intro':
            ...
            destination = os.path.join(self.build_path, base, 'index.html')
        else:
...
def build():
    ...
    writer = HTMLWriter(
        BUILD_PATH,
        Template(codecs.open('templates/base.html', encoding='utf').read()))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Build 140cRPG and other tools')
